...
public class BoardPage {
	...
	public static String pagingStr(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
		...
		/*
		 * 현재 페이지 블럭에서 사용할 첫번째 페이지번호를 계산한다. 현재 1페이지라 가정하면 (((1-1) / 5) * 5) + 1 = 1 현재
		 * 5페이지라 가정하면 (((5-1) / 5) * 5) + 1 = 1 ==> 즉 1~5페이지인 경우 해당블럭의 첫번째 페이지는 1이다. 현재
		 * 6페이지라 가정하면 (((6-1) / 5) * 5) + 1 = 6 현재 11페이지라 가정하면 (((11-1) / 5) * 5) + 1 =
		 * 11
		 */
		...
		/*
		 * 각 페이지 번호로 바로가기 링크 출력 앞에서 계산한 pageTemp를 blockPage만큼 반복해서 출력한다. 이때 1씩 증가시켜준다. 즉
		 * 한블록당 5페이지를 출력하게된다.
		 */
		int blockCount = 1;
	...
	public static String pagingImg(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
