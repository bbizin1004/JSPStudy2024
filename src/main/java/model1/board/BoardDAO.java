...
public class BoardDAO extends JDBConnect {
	...
	/*
	 * 작성된 게시물을 인출하여 반환한다. 특히 반환값은 여러개의 레코드를 반환할 수 있고, 순서를 보장해야 하므로 List 컬렉션을 사용한다.
	 */
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			/*
			 * 인파라미터가 있는 동적쿼리문을 작성한 후 유저가 입력한 값으로 설정한다. 일련번호는 시퀀스로 자동부여하고, 조회수는 0으로 입력한다.
			 */
			...
			// 쿼리문을 실행하여 입력처리 한 후 결과값은 정수로 반환받는다.
	...
	public BoardDTO selectView(String num) {
		...
		//게시물 레코드를 저장하기 위한 DTO
		BoardDTO dto = new BoardDTO();
		
		/*회원의 이름까지 인출하기 위해 member 테이블과 내부조인을
		 설정하여 레코드를 가져온다.*/
