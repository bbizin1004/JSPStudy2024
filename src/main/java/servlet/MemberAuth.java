...
public class MemberAuth extends HttpServlet {
	...
	이때 최초 1번만 호출된다. 여기서는 DB연결 처리를 한다. 
	*/
	@Override
	public void init() throws ServletException {
		
		/*
		서블릿 내에서 application 내장객체를 얻어온다. 모델2방식에서는
		...
		*/
		...
		//위 정보를 통해 DAO인스턴스를 생성하여 DB에 연결한다.
		dao = new MemberDAO(driver, connectUrl, oId, oPass);
	}
	
	
	/*
	서블릿의 수명주기 메서드 중에서 요청을 먼저 받은 후 doget() 혹은 
	doPost()로 분기하는 역활의 메서드이다. 따라서 get/post 두가지 방식 모두 
	이 메서드로 받은 후 요청을 처리할 수 있다.
	*/
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		*/
		...
		회원인증을 진행한 후 일치하는 정보가 있다면 회원레코드를 DTO에 
		저장한 후 반환한다.
		*/
		...
		if (memberName != null) {
			/*
			DTO객체에 회원의 이름이 저장되어 있다면 로그인에 성공한
			경우로 판단할 수 있다.
			*/
