...
/*
getTime() : 날짜를 1970년 1월 1일부터 지금까지의 시간을 초단위로 변환하여
	반환한다. 이를 타임스템프 라고 한다.
*/
long add_date = s.parse(request.getParameter("add_date")).getTime();

/*
문자열로 전송되는 값을 실제 날짜로 변경하기 위해 포맷을 지정한 후 타임스탬프로 
...
/*
addDateHeader() : 응답헤더에 날짜형식을 추가하는 경우 long타입의 타임스템프로 
	변환 후 추가한다.
...
//정수 형식 추가. 동일한 헤더명으로 2개의 값이 추가된다.
...
/*웹브라우저가 인식하지 못하는 컨텐츠타입을 응답헤더에 설정하면 다운로드창을
띄우게 된다.*/
...
<body>
	...
	/*
	출력결과에서 날짜의 경우 하루전인 11월 30일이 출력된다. 이것은 대한민국이 
	세계표준시(그리니치 천문대)에 +09 즉 9시간이 빨라서 생기는 현상으로 
	9시간을 더해줘야 정상적인 날짜가 출력된다.
	
	myNumber라는 헤더명이 2번 출력되는데 8282만 나오는걸 볼수 있다. 이것은
