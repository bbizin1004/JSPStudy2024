...
/*
전체페이지수를 계산
전체케시물 갯수/ 페이지당 출력할 게시물갯수 => 결과값의 올림처리
가령 게시물 갯수가 108개라면 10으로 나웠을때 10.8이 되므로 올림처리하여 
11을 만들어준다. 즉 11페이지가 된다. 
만약 totalCount를 double형으로 변환하지 않으면 정수의 결과가 나오게 되므로
10페이지가 될것이다. 따라서 실수의 결과를 얻기위해 형변환 후 계산한다.
*/
...
쿼리문에 적용할 start와 end를 페이지 번호와 페이지 사이즈를 통해 계산한 후 
...
<%
	...
	/*출력할 게시물이 있는 경우에는 확장for문을 List에 저장된 레코드의
	갯수만큼 반복한다.*/
	//게시물의 가상번호
	int virtualNum = 0;
	int countNum = 0;
	...
	{ 
		/*
		현재 출력할 게시물의 갯수에 따라 번호가 달라기제 되므로
		아래와 같이 가상번호를 부여한다.
		*/
