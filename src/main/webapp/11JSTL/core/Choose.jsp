...
<body>
<!-- 
choose태그 : 하위에 when, otherwise 태그를 가지고 있다. 여러 조건이
	있는 경우 사용하기에 적합하다.
 -->
	...
	또한 when 과 otherwise 사이에도 기술 할 수 없다.
	if태그와 동일하게 test속성에 조건을 기술한다. 또한 공백(스페이스)이
	들어가면 무조건 false로 판단하므로 주의해야한다.
	-->
	<c:choose>
		...
		<c:otherwise>
		<!-- else와 동일한 역활을 한다. 위 조건에 만족되지 않을때
		실행된다.  -->
			${number }는 홀수입니다.
		</c:otherwise>
	</c:choose>
	...
	<!-- form태그에 아무런 속성이 없으면 get방식으로 현재페이지로 
	폼값을 전송한다. -->
	<form>
	...
	<!-- 
	3과목의 점수를 합산 후 평균을 구한다. 결과는 avg에 저장된다.
	EL은 자동형변환을 지원하므로 경우에 따라 실수의 결과가 나올 수 있다.
	-->
	<c:if test="${ not (empty param.kor or empty param.eng or empty param.math) }">
		<c:set var= "avg" value="${(param.kor + param.eng + param.math) /3}" />
		평균 점수는 ${avg }으로
		
		<!-- 평균점수를 통해 조건을 걸어 학점을 출력한다. when태그는 if 혹은
		if else와 동일하고 otherwise는 else와 동일하다. -->
		<c:choose>
