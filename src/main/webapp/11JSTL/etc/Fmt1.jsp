...
<head>
<meta charset="UTF-8">
<title>JDTL= fmt 1</title>
</head>
<body>
	<!--  
국제화(Formatting) 태그
	: 국각별로 다양한 언어, 날짜, 시간, 숫자형식을 지정할때 사용한다.
	접두어로는 fmt를 사용한다.
-->
	<h4>숫자 포맷 설정</h4>
	<!-- 변수생성 -->
	...
	<!--  
	groupingUsed : 세자리마다 콤마를 출력한다. true가 디폴트이므로
	콤마를 출력하지 않는다면 false로 지정해야 한다.
	-->
	콤마 O: <fmt:formatNumber value="${ number1 }" /><br />
	콤마 X: <fmt:formatNumber value="${ number1 }" 
	 			groupingUsed="false" /><br />
	 
	 <!--  
	 type: currency인 경우 현지 통화기호로 출력된다.
	 currencySymbol: 통화기호를 변경할 수 있따.
	 var: 이 속성이 있으면 태그가 작성된 위치에서는 바로 출력되지 않고,
	 	EL로 원하는 곳에서 출력할 수 있다. 만약 이 속성이 없으면 태그가 
	 	있는 위치에 즉시 출력된다.
	 -->
